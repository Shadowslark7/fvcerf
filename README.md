...
* API изначально сломаны. Классический пример: file = open(); close(file); read(file); WTF?!
...
* Это CAP-теорема - если кратко.
* Если программе не хватает 640kb, значит она не может отскейлиться на другие машины. И тогда ей наступает пизда.
