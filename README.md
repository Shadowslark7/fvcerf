# Logh-Broker©®™ Whitepaper (туалетная бумага rus.) #

## Abstract ##
Logh-broker©®™ (Лох-брокер [ləʊh ˈbrəʊkə]) - высоэффективный комплекс программного обеспечения, предназначенный для перекладывания из пустого в порожнее.
...
### Основные концепции: ###
 ...
 2. Protocols. Только Протоколы, никаких API, RPC и вот этой всей херни.
 3. No over subscription. Все по-честному и без сюрпризов (другими словами, плачу - и плачу).
 4. Don't break laws of physics.
 5. 640kb is enough for everyone -- Bill Gates (who knows really?..)

### Motivation ###
*Нумерация в соответствии с основными концепциями

1. А иначе - зачем все это делать? : )
2. API изначально сломаны. Классический пример: file = open(); close(file); read(file); WTF?!
3. У вас 10 джобов на 8 воркеров... Выберите лучший из 10x8 возможных вариантов, выберите следующий из 9x8 вариантов... 10!x8^10 = 3.7x10^16. Выглядит как NP hard задача.
4. Это CAP-теорема - если кратко.
5. Речь не столько о 640kb памяти, сколько о ресурсах. Они в принципе должн быть ограничены. Если программе не хватает 640kb, значит она не может отскейлиться.
 ...
 * Почему так? Разные причины: решают задачу очереди сообщений в общем виде, изначально делали систему совсем не для этого, просто не получилось, whatever.
 ...
 * Надежно писать на диск? Звать fsync.
 ...
 * Обновлять С/С++ модули? Выделять критический функционал в модули, которые Fast and Robust©®™ и не требуют обновления. Lol.
...
* Disk node пишет данные из сокета ~~~в файл~~~ на диск и форвардит данные дальше по сети.
...
* Почему управляется из вне? Потому что, когда нужно что-то сохранить, возникает два самых сложных вопроса: какое имя будет у файла и в какую директорию его положить. И это не забота этого сервиса.
* Таких таких штук должно быть две: одна пишет, другая читает; или одна? Single one vs Command Query Resposibility Segregation (SQRS).
